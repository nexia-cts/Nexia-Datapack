...
execute at @a[tag=bot,scores={attack=..0}] run execute at @a[distance=0.1..3.0,sort=nearest,tag=bot_target] run schedule function core:bot/attack3 2t
execute at @a[tag=bot,scores={attack=..0,r=..0}] run execute at @a[distance=3.0..4.0,sort=nearest,tag=bot_target] run schedule function core:bot/attack3 2t
